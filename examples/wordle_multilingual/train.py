"""Wordle Multilingual demo — RL training of gpt-oss-120b LoRA on **Japanese** Wordle (4× H100).

Mirrors examples/wordle/train.py, with three changes:
  1. MCP server is the unified multilingual one, spawned with `--mode japanese`.
  2. Dataset targets come from data/japanese/answers.txt (7,910 katakana words).
  3. Prompt / dev_instructions / wandb run name reflect the Japanese setup.

Recipe is identical to the English run: DDIS loss, asymmetric trust region,
LoRA r=8 on attention, routing replay ON, DFlash speculative decoding,
merged weight sync.
...
Quick start:
    bash examples/wordle_multilingual/serve.sh        # Wait for "/health 200" before starting training
...
RUN_DIR = os.environ.get("RUN_DIR", str(EXAMPLE_DIR.parent.parent / "runs" / "wordle_ja_demo"))
...
def make_dataset() -> list[dict]:
    """One item per katakana target; each item spawns its own Wordle MCP server in japanese mode."""
    words = [w.strip() for w in WORDLE_ANSWERS.read_text(encoding="utf-8").splitlines() if w.strip()]
    logger.info(f"Loaded {len(words)} Japanese Wordle target words")
    return [
        {
            "prompt": "Play Japanese Wordle. Guess the 5-character katakana word.",
            "target_word": word,
            "mcp_config": {
                "mcpServers": {
                    "wordle": {
                        "command": sys.executable,
                        "args": [WORDLE_SERVER, "--mode", WORDLE_MODE, "--target", word, "--no-banner"],
                    }
                }
            },
        }
...
def main():
    training_config = TrainingConfig(
        ...
        wandb_run_name="wordle-ja-demo",
        wandb_notes="gpt-oss-120b LoRA r=8 on Japanese (katakana) Wordle, 4× H100, DDIS 0.2/0.28.",
    )
    ...
    flow = RLFlow(
        ...
        conv_config=ConversationConfig(
            ...
            dev_instructions=(
                "You are playing Japanese Wordle. The answer is a 5-character katakana string "
                "corresponding to a real Japanese word (often written in kanji). "
                "Use the 'guess' tool with a 5-character katakana string. "
                "Each guess returns per-kana feedback (correct / present / absent). "
                "You have 6 attempts. Invalid input (wrong length or not in the kana dictionary) "
                "still consumes one attempt."
            ),
        ),
    ...
    logger.info("Starting RLFlow training (Wordle Japanese demo)")
