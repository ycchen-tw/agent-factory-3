"""MCP HTTP Server with subprocess Python execution (no bwrap).

For environments where bwrap/user namespaces are unavailable (e.g., Kaggle).
Same protocol and session management as server_v4p1.py, but spawns plain
Python subprocesses instead of bwrap-sandboxed ones.

Security is NOT enforced — the subprocess has full access to the host
filesystem and network. Only use in already-sandboxed environments.

Usage:
    python server_subprocess.py --port 8811
    python server_subprocess.py --port 8811 --timeout 20 --max-output 2000
...
PYTHON_BIN = os.environ.get("PYTHON_TOOL_PYTHON", sys.executable)
...
async def _spawn_session(session_id: str) -> Session:
    """Spawn a plain Python subprocess running the executor."""
...
async def lifespan(server):
    ...
    logger.info(
        ...
        f"executor={EXECUTOR_PATH}, mode=subprocess [no bwrap])"
    )
    try:
        yield {}
    finally:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MCP Python Tool Server (subprocess, no bwrap)")
    parser.add_argument("--port", type=int, default=8811)
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--timeout", type=float, default=None,
                        help=f"Override PYTHON_TOOL_TIMEOUT (default: {_default_timeout})")
    parser.add_argument("--max-output", type=int, default=None,
                        help="Override PYTHON_TOOL_MAX_OUTPUT_CHARS")
    parser.add_argument("--python", default=None,
                        help="Python binary for executor subprocess (default: sys.executable)")
    parser.add_argument("--log-level", default="INFO")
    args = parser.parse_args()
    if args.timeout is not None:
        TIMEOUT = args.timeout
        OUTER_TIMEOUT = TIMEOUT + 10.0
        os.environ["PYTHON_TOOL_TIMEOUT"] = str(TIMEOUT)
    if args.max_output is not None:
        MAX_OUTPUT_CHARS = str(args.max_output)
        os.environ["PYTHON_TOOL_MAX_OUTPUT_CHARS"] = MAX_OUTPUT_CHARS
