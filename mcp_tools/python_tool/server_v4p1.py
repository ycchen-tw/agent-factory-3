"""MCP HTTP Server with bwrap-sandboxed Python execution (v4p1).

Same as v4 but with local-disk sandbox venv to avoid WekaFS bind mounts in
bwrap namespaces. This prevents kernel namespace_sem lock contention during
bwrap cleanup that caused ~55s system freezes when WekaFS enters D-state.

Key change: bwrap uses selective --ro-bind of local paths only (no --ro-bind / /).
The sandbox Python and packages live in $SANDBOX_VENV (default: /tmp/sandbox_venv),
which is on local disk, not WekaFS.

Usage:
    SANDBOX_DIR=/tmp/sandbox_python uv run python mcp_tools/python_tool/server_v4p1.py --port 8811

Environment variables (in addition to v4's):
...
# ---------------------------------------------------------------------------
# bwrap command — local-only bind mounts, no WekaFS
# ---------------------------------------------------------------------------


def _build_bwrap_cmd() -> list[str]:
    """Build bwrap command using only local-disk bind mounts.

    Instead of --ro-bind / / (which pulls in WekaFS mounts and causes kernel
    namespace_sem contention on cleanup), we bind only the paths the sandbox
    Python actually needs:
      - /usr, /lib, /lib64, /bin, /sbin, /etc — system libraries and binaries
      - $SANDBOX_VENV — sandbox Python + packages (on local /tmp)
    """
    ...
    if not SANDBOX_PYTHON.exists():
        raise RuntimeError(
            ...
            f"Run serve_python_tool_v2.sh to build the sandbox env first."
        )
    ...
    # System libraries and binaries (local disk, not WekaFS)
    ...
    # Sandbox dir AFTER --tmpfs /tmp (bwrap applies mounts in order;
    # --tmpfs /tmp would hide a prior bind of /tmp/sandbox_python)
    cmd += ["--ro-bind", str(SANDBOX_DIR), str(SANDBOX_DIR)]
    ...
    cmd += [
        "--",
        str(SANDBOX_PYTHON), "-u",    # unbuffered
        str(SANDBOX_EXECUTOR),
    ]
...
# Environment for sandbox — clean, no WekaFS paths in LD_LIBRARY_PATH
BWRAP_ENV = {
    "HOME": "/tmp",
    ...
    # Prevent OpenBLAS from spawning 64 threads that exceed RLIMIT_DATA in bwrap
    "OPENBLAS_NUM_THREADS": "4",
    "OMP_NUM_THREADS": "4",
    # Silence tqdm progress bars (import works, output suppressed)
    "TQDM_DISABLE": "1",
    # Force system libstdc++/libgcc_s instead of WekaFS HPC paths
    "LD_LIBRARY_PATH": "/usr/lib64:/lib64",
}


# ---------------------------------------------------------------------------
# Session management (unchanged from v4)
# ---------------------------------------------------------------------------
...
async def kill_session(session_id: str) -> None:
    ...
    try:
        ...
        logger.debug(f"kill_session {session_id[:12]}... wait timeout (bwrap may be zombie)")
    _stats_killed += 1
...
# ---------------------------------------------------------------------------
# Idle reaper + disconnect detection (unchanged from v4)
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MCP Python Tool Server (bwrap, local sandbox)")
    ...
    if not SANDBOX_PYTHON.exists():
        logger.error(f"Sandbox Python not found at {SANDBOX_PYTHON}. "
                     f"Run serve_python_tool_v2.sh first.")
        sys.exit(1)

    logger.info(f"Using sandbox dir: {SANDBOX_DIR}")
    logger.info(f"bwrap command: {' '.join(BWRAP_CMD[:10])}...")
